...
class Line:
    ...
    def __calk_vectors(self, point: Point):
        # Преобразуем входные данные в numpy массивы
        point = np.array([point.x, point.y, point.z])
        line_point = np.array([self.s_point.x, self.s_point.y, self.s_point.z])
        line_direction = np.array([self.e_point.x, self.e_point.y, self.e_point.z])
        return point, line_point, line_direction
