...
class DeformationGMMClassificator:

    def __init__(self, def_scan, n_of_class=3, probability_delta=0.0):
        self._def_scan = def_scan
        self._n_of_class = n_of_class
        self._probability_delta = probability_delta
        self._def_arr = None
    ...
    def __iter__(self):
        return iter(self._def_scan)

    def __str__(self):
        return (f"{self.__class__.__name__} "
                f"[def_scanl: {repr(self._def_scan)}\tn_of_class: {self._n_of_class}\t"
    ...
    def _init_defs_arr(self):
        def_arr = []
        for point in self:
            if point.deformation is not None:
                def_arr.append(point.deformation)
        def_arr = np.array(def_arr)
        self._def_arr = def_arr

    def _init_classificator(self):
        if self._def_arr is None:
            self._init_defs_arr()
        self._gmm_classificator = GaussianMixture(self._n_of_class).fit(self._def_arr.reshape(-1, 1))
        self._init_class_params_dict()
        self._predict_defs_classes()
    ...
    def _chose_class_indexes(self):
        ...
        for idx, class_params in self._class_params_dict.items():
            # pass
            if abs(class_params["M"]) <= self._class_params_dict[self._stable_zone_idx]["Std"] * 2.5:
                class_params["type"] = 0

    def _get_deformation_type_from_prob_lst(self, prob_lst):
        idx = prob_lst.index(max(prob_lst))
        return self._class_params_dict[idx]["type"]

    def _check_probability_values(self, prob_lst):
        sort_prob_lst = sorted(prob_lst)
        max_prob, second_max_prob = sort_prob_lst[-1], sort_prob_lst[-2]
        if max_prob - second_max_prob >= self._probability_delta:
            return True
        return False

    def _predict_defs_classes(self):
        for point in self:
            if point.deformation is not None:
                point.classes_probability = self.predict_classes_probability_for_deformation(point.deformation)[0]
                prob_lst = list(point.classes_probability)
                if self._check_probability_values(prob_lst):
                    point.deformation_class = self._get_deformation_type_from_prob_lst(prob_lst)
                else:
                    point.deformation_class = None
            else:
                point.classes_probability = None
                point.deformation_class = None

    def predict_classes_probability_for_deformation(self, deformation):
        return self._gmm_classificator.predict_proba([[deformation]])

    def predict_deformation_type_for_deformation(self, deformation):
        prob_lst = self.predict_classes_probability_for_deformation(deformation)
        return self._get_deformation_type_from_prob_lst(prob_lst)
    ...
    def plot_classification_result(self, n_for_aic_bic=10):
        ...
        x = np.linspace(min(self._def_arr), max(self._def_arr), 1000)
